...
void Lvgl_Example1(void){
  ...
  // Custom tabview indicator line style (bold)
  lv_obj_t * tab_btns = lv_tabview_get_tab_btns(tv);
  lv_obj_set_style_border_width(tab_btns, 3, LV_PART_ITEMS | LV_STATE_CHECKED);  // Set indicator line width to 4 pixels
  lv_obj_set_style_border_color(tab_btns, lv_color_hex(0x000000), LV_PART_ITEMS | LV_STATE_CHECKED);  // Set indicator color
  lv_obj_set_style_border_side(tab_btns, LV_BORDER_SIDE_BOTTOM, LV_PART_ITEMS | LV_STATE_CHECKED);  // Indicator at the bottom
  
  // Set "LAFVIN" tab text color to #00a0e9 and make it bold and larger
  lv_color_t tab_text_color = lv_color_hex(0x007bba);  // New blue
  ...
  // Increase letter spacing for a bolder visual effect
  ...
  // Add text shadow to make the font appear bolder (multi-layer overlay effect)
...
void Lvgl_Example1_show(void) {
  if (tv != NULL) {
    ...
    printf("LVGL interface displayed\n");
  }
}

// New: Set screen background to black (to avoid white screen flicker)
...
static void Onboard_create(lv_obj_t * parent)
{
  ...
  lv_obj_set_style_bg_opa(panel1, LV_OPA_COVER, 0);               // Fully opaque background
  ...
  // Column width and spacing: fixed label column width, allowing more space for text boxes
  static lv_coord_t grid_2_col_dsc[] = {20, 3, LV_GRID_FR(1), LV_GRID_TEMPLATE_LAST}; // Label fixed 60px | 3px spacing | Text box occupies remaining space
  ...
  // Title occupies the entire row (spans 3 columns: label + spacing + text box)
  lv_obj_set_grid_cell(panel1_title, LV_GRID_ALIGN_START, 0, 3, LV_GRID_ALIGN_CENTER, 0, 1);

  // SD Card row: label on the left (column 0), text box on the right (column 2)
  ...
  // Flash Size row: label on the left (column 0), text box on the right (column 2)
  ...
  // Runtime row: label on the left (column 0), text box on the right (column 2)
  ...
  // Wireless occupies the entire row (spans 3 columns)
...
void IRAM_ATTR example1_increase_lvgl_tick(lv_timer_t * t)
{
  ...
  // Calculate runtime (time since boot)
  unsigned long runtime_ms = millis();  // Get milliseconds since boot
